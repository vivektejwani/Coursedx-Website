	...
	else
	{
		...
		if(isset($_POST['login'])&&isset($_POST['password']))
		{
			...
			if(!empty($log_id)&&!empty($pwd))
			{
				...
				$query1="SELECT `login` from student where `login`='".$log_id."'";
				...
				{
					$query2="SELECT `password` from student where `login`='".$log_id."'";
					$query_result2=mysql_query($query2);
					$pwd_found=mysql_result($query_result2,0,`password`);
					if($pwd_found==md5($pwd))
					{
						$query3="SELECT `s_id` from student where `login`='".$log_id."'";
						$query_result3=mysql_query($query3);
						$id=mysql_result($query_result3,0,`s_id`);
						$_SESSION['id']=$id;
						$_SESSION['type']=='s';
						header('Location: student_home/index.php');
					}
...
<head>
  ...
  <title>Coursedx Student Login</title>
...
<h1 style="font-size: 30px">&nbsp;</h1>
<h1 style="font-size: 30px">Welcome to COURSEDX Student Login!</h1>
<hr>
<hr>
<hr>
<form method="post" action="student_login.php" class="login">
    <p>
    ...
    <p class="forgot-password"><a href="new_reg_student.php">New Registration?</a>  </p>
